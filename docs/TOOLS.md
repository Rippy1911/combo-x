...
Read tools never end at an unexplained cut-off. Every list reply carries
`total` / `nextOffset` / `hasMore`, and truncation drops whole items (never
characters) behind a `_truncated` envelope that names the count and the exact
follow-up call. `get_interactive` indices are absolute over the full scan, so
filtering or paging never invalidates a `click_index`.

`RepeatGuard` refuses a third byte-identical call+result and suggests
alternatives; `navigate` flags `redirected: true` with the requested URL when
the landing page differs. Rationale, the incident that motivated it, and the
prompt-cache ordering rules: `docs/AGENT_PAGING_AND_CACHE.md`.
